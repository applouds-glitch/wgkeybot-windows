...
// SaveConfig шифрует конфиг через DPAPI и сохраняет в DefaultConfigDir
// с расширением .wgkbot. Файл нечитаем без Windows-аккаунта пользователя.
func SaveConfig(token string, data []byte) (string, error) {
	dir := DefaultConfigDir()
	if err := os.MkdirAll(dir, 0700); err != nil {
		return "", fmt.Errorf("create config dir: %w", err)
	}
	name := sanitizeTokenName(token)
	path := dir + `\` + name + ".wgkbot"
...
// sanitizeTokenName возвращает безопасное имя файла на основе токена.
func sanitizeTokenName(token string) string {
